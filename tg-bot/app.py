...
async def start(update: Update, context) -> None:
    await update.message.reply_text('Привет! Я могу составить новый рецепт пиццы! Напиши мне ингредиенты :)')
...
async def handle_message(update: Update, context):
    ...
    try:
        ...
        response = ollama.chat(model='llamapizza:latest', messages=context_memory[user_id])
